...
db.serialize(() => {
  ...
  db.run(`ALTER TABLE progress ADD COLUMN lastActiveDate TEXT DEFAULT NULL`, (err) => {});
  db.run(`ALTER TABLE progress ADD COLUMN streakCount INTEGER DEFAULT 0`, (err) => {});
...
app.post('/api/login', (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE name = ? AND password = ?", [name, password], (err, user) => {
    ...
    if (role === 'student') {
      ...
      db.get("SELECT lastActiveDate, streakCount FROM progress WHERE userId = ?", [user.id], (err, row) => {
        if (!err && row) {
          ...
          if (lastDate !== today) {
            ...
            db.run("UPDATE progress SET lastActiveDate = ?, streakCount = ? WHERE userId = ?", [today, newStreak, user.id]);
          }
...
app.get('/api/progress/:userId', (req, res) => {
  db.get("SELECT * FROM progress WHERE userId = ?", [req.params.userId], (err, row) => {
    ...
    const streakCount = row.streakCount || 0;
